...
class Classifier:
    ...
    def __init__(
        ...
        self.model = self.build_classifier(model)

        if output_model_structure is True:
            keras.utils.plot_model(
                self.model,
                to_file=output_directory / "plot_model_structure.png",
                show_shapes=True,
                show_layer_names=True,
            )

    def build_classifier(self, model, **kwargs):
        """
        ...
        **kwargs: Keyword arguments for tf.keras.Model.compile method
        ...
        model.compile(
            loss=self.loss, optimizer=self.optimizer, metrics=metrics, **kwargs
        )
    ...
    def get_class_weight(y: np.ndarray) -> dict:
        ...
        class_weights = class_weight.compute_class_weight(
            class_weight="balanced", classes=np.unique(label_indices), y=label_indices
        )
    ...
    def fit_classifier(self, train, valid, **kwargs):
        ...
        **kwargs: Keyword arguments for tf.keras.Model.fit method
        ...
        model_checkpoint = keras.callbacks.ModelCheckpoint(
            filepath=str(self.output_directory / "best_model.weights.h5"),
            save_weights_only=True,
            monitor=self.monitor,
            save_best_only=True,
        )
        tensorboard_callback = keras.callbacks.TensorBoard(
            log_dir=self.output_directory / "classifier" / "logs", histogram_freq=1
        )
...
class ToCategoricalMetric(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, **kwargs):
        ...
        if self.prob_dim:
            y_pred = tf.cast(y_pred[:, self.prob_dim], tf.float32)
        else:
