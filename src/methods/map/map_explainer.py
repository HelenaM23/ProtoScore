...
class Explainer:
    ...
    def fit_explainer(self, classifier, X, explainer_name):
        ...
        for epoch in range(self.epochs):
            ...
            epoch_loss_metric = tf.keras.metrics.Mean(name="epoch_total_loss")
            epoch_sep_reg_metric = tf.keras.metrics.Mean(name="epoch_sep_reg_loss")
            epoch_mse_metric = tf.keras.metrics.Mean(name="epoch_mse_loss")
            epoch_ce_metric = tf.keras.metrics.Mean(name="epoch_ce_loss")
            ...
            for step in range(bat_per_epoch):
                ...
                with tf.GradientTape() as tape:
                    ...
                    # Compute losses
                    sep_reg_new = (
                        self.cluster_regularization(x_batch_train) * self.sep_loss
                    )
                    mse_loss_val = mse_loss_fn(x_batch_train, x_reconstructed)
                    ce_loss_val = ce_loss_fn(y_batch_train, y_pred_reconstructed)
                    total_loss = mse_loss_val + ce_loss_val + sep_reg_new

                grads = tape.gradient(total_loss, self.explainer.trainable_weights)
                optimizer.apply_gradients(zip(grads, self.explainer.trainable_weights))

                # Update epoch-level metrics
                epoch_loss_metric.update_state(total_loss)
                epoch_sep_reg_metric.update_state(sep_reg_new)
                epoch_mse_metric.update_state(mse_loss_val)
                epoch_ce_metric.update_state(ce_loss_val)

                # Optional: print step-level info if you want
                if step % 100 == 0:
                    print(
                        f"Step {step} - total_loss: {total_loss.numpy():.4f}, "
                        f"sep_reg: {sep_reg_new.numpy():.4f}, "
                        f"mse_loss: {mse_loss_val.numpy():.4f}, "
                        f"ce_loss: {ce_loss_val.numpy():.4f}"
                    )

            # At the end of the epoch, log or save the metrics
            with summary_writer.as_default():
                tf.summary.scalar(
                    "Total Loss per Epoch", epoch_loss_metric.result(), step=epoch
                )
                tf.summary.scalar(
                    "Separation Regularization per Epoch",
                    epoch_sep_reg_metric.result(),
                    step=epoch,
                )
                tf.summary.scalar(
                    "MSE Loss per Epoch", epoch_mse_metric.result(), step=epoch
                )
                tf.summary.scalar(
                    "Cross-Entropy Loss per Epoch", epoch_ce_metric.result(), step=epoch
                )

            print(
                f"End of epoch {epoch}: "
                f"total_loss: {epoch_loss_metric.result().numpy():.4f}, "
                f"sep_reg: {epoch_sep_reg_metric.result().numpy():.4f}, "
                f"mse_loss: {epoch_mse_metric.result().numpy():.4f}, "
                f"ce_loss: {epoch_ce_metric.result().numpy():.4f}"
            )

            # (Optional) If you want a CSV record of just the epoch number (or metrics)
            pd.DataFrame(
                {
                    "epoch": [epoch],
                    "total_loss": [epoch_loss_metric.result().numpy()],
                    "sep_reg": [epoch_sep_reg_metric.result().numpy()],
                    "mse_loss": [epoch_mse_metric.result().numpy()],
                    "ce_loss": [epoch_ce_metric.result().numpy()],
                }
            ).to_csv(
                self.output_directory / "epoch_metrics.csv",
                mode="a",
                header=not (self.output_directory / "epoch_metrics.csv").exists(),
                index=False,
            )
        if not self.explainer.built:
            self.explainer.build((None,) + self.input_shape)
        self.explainer.save_weights(str(self.output_directory / "ae_weights.weights.h5"))
    ...
    def cluster_regularization(self, X):
        ...
        kmeans = KMeans(n_clusters=self.n_concepts, random_state=0).fit(
            all_latent.numpy()
        )
    ...
    def separation_regularization(self, concepts):
        ...
        L_sparse_2_new = (
            tf.reduce_sum(all_concept_dot * mask)
