...
class Encoder(layers.Layer):
    def __init__(self, latent_dim, name="encoder", **kwargs):
        ...
        self.dense = layers.Dense(
            units=latent_dim, activity_regularizer=regularizers.l1()
        )
...
class Decoder(layers.Layer):
    def __init__(self, original_dim, name="decoder", **kwargs):
        ...
        self.stride = 2
        self.dense = layers.Dense(
            units=original_dim[0] / self.stride * original_dim[1] / self.stride * 64
        )
    ...
    def call(self, inputs):
        ...
        x = tf.reshape(
            ...
            (
                -1,
                int(self.original_dim[0] / self.stride),
                int(self.original_dim[1] / self.stride),
                64,
            ),
        )
        x = self.conv1(x)
...
class AutoEncoder(keras.Model):
    def __init__(self, original_dim, latent_dim=5, name="autoencoder", **kwargs):
