...
class ConceptProperties:
    ...
    def calc_completeness_given_concept(
        self, predictor, decoder, latent, concepts, y_pred, used_concepts_index
    ):
        """
        excluding concept, by setting picking closest other concept
        """
        concept_index_not_excluded = self.get_concept_not_excluded(
            concepts, used_concepts_index
        )
        # map each instance to concepts of subsets
        instance_concept = self.get_closest_concept_to_instances(
            latent, concepts[concept_index_not_excluded]
        )
        if decoder:
    ...
    def get_completness(y_classifier, y_explainer):
        completeness = sum(
            np.argmax(y_classifier, axis=1) == np.argmax(y_explainer, axis=1)
    ...
    def get_concept_not_excluded(concepts, used_concepts_index):
        ...
        concept_index_not_excluded = [
            used_concepts_index[int(i)] for i in neighbour_index
        ]
    ...
    def get_concept_shap(self, predictor, decoder, latent, concepts, y_pred):
        ...
        for idx in c_id:
            ...
            for subset in subsets:
                # score 1:
                c1 = subset + [idx]
                score1 = self.calc_completeness_given_concept(
                    predictor, decoder, latent, concepts, y_pred, c1
                )

                # score 2:
                c1 = subset
                if c1 != []:
                    score2 = self.calc_completeness_given_concept(
                        predictor, decoder, latent, concepts, y_pred, c1
                    )
                else:
                    score2 = 0
                ...
                norm = (
                    math.factorial(len(c_id) - len(subset) - 1)
    ...
    def KL_divergence_performance(X, latent):
        ...
        P = _joint_probabilities(
            distances=distances, desired_perplexity=perplexity, verbose=False
        )

        distances_latent = pairwise_distances(latent, metric="euclidean", squared=True)
        Q = _joint_probabilities(
            distances=distances_latent, desired_perplexity=perplexity, verbose=False
        )
